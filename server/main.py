...
def clean_iptables(address: str ):
  ...
  if len(matches) > 1:
    subprocess.check_output(f'docker exec -t wireguard iptables -D INPUT {matches[0].split(" ")[0]}')
  else:
    for match in matches:
      subprocess.check_output(f'docker exec -t wireguard iptables -D INPUT {match.split(" ")[0]}')
